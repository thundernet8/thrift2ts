...
header += `import Thrift from "browser-thrift2"\r\n`
header += `import IProtocol, { ProtocolClass } from "browser-thrift2/src/thrift/interface/IProtocol"\r\n`
header += `import ITransport, { TransportClass } from "browser-thrift2/src/thrift/interface/ITransport"\r\n\r\n`
...
export default (ast: any): string => {
    ...
    const includesHandler = (includes: object[]): string => {
        ...
        Object.keys(includes).map(key => includes[key]).forEach(include => {
            imports.push(`\r\nimport * as ${getThriftFileName(include.value)} from "${thrift2TsPath(include.value, false, false)}";\r\n`);
        });
        return imports.join('');
    }
...
export const clientsIndex = (clientsDict) => {
    ...
    files.forEach(file => {
        ...
        if (clients.length > 0) {
            ...
            codes.push(`import { ${clients.map(x => x + 'Client')} } from "./${file}"\r\n`)
        }
    })

    // export
    codes.push(`\r\n\r\nexport default {\r\n${allClients.map(x => '    ' + x + ': ' + x + 'Client').join(',\r\n')}\r\n}\r\n`)
