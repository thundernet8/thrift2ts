...
export default (ast: any, Request = './request'): string => {
    ...
    const servicesHandler = (services: object[]): string => {
        ...
        Object.keys(services).forEach(key => {
            code += serviceArgsndResultsHandler(key, services[key])
    ...
    const serviceArgHandler = (service, func): string => {
        ...
        if (args && args.length) {
            ...
            args.forEach((item, index) => {
                ...
                if (mapThriftType(item['type']) === 'struct') { // TODO check more
        ...
        if (args && args.length) {
            ...
            args.forEach((item, index) => {
                ...
                if (mapThriftType(type) === 'struct') {
                    codes.push(`this.${item['name']} = new ${type}()\r\n                        this.${item['name']}.read(input)`)
                } else {
    ...
    const serviceResultHandler = (service, func): string => {
        ...
        if (func['type'] !== 'void') {
            ...
            throws.forEach((item, index) => {
                codes.push(`\r\n        if (args instanceof ${item['type']}) {\r\n            this.${item['name']} = args\r\n            return\r\n        }`);
            })
            codes.push('\r\n        if (args) {');
            codes.push(`\r\n            if (args.success !== undefined && args.success !== null) {\r\n                this.success = args.success\r\n            }`);
            throws.forEach((item, index) => {
                codes.push(`\r\n            if (args.${item['name']} !== undefined && args.${item['name']} !== null) {\r\n                this.${item['name']} = args.${item['name']}\r\n            }`);
            })
            codes.push('\r\n        }');
        }
        // close constructor
        ...
        if (func['type'] !== 'void') {
            codes.push('\r\n            switch (fid) {')
            codes.push(`\r\n                case 0:\r\n                    if (ftype === ${getThriftTypeStr(func['type'])}) {\r\n                        this.success = input.${getReaderStr(func['type'])}\r\n                    } else {\r\n                        input.skip(ftype)\r\n                    }\r\n                    break`)

            if (throws && throws.length) {
                throws.forEach((item, index) => {
                    codes.push(`\r\n                case ${index + 1}:\r\n                    if (ftype === ${getThriftTypeStr(item['type'])}) {\r\n                        this.${item['name']} = new ${item['type']}()\r\n                        this.${item['name']}.read(input)\r\n                    } else {\r\n                        input.skip(ftype)\r\n                    }\r\n                    break`)
                })
            }
            codes.push('\r\n                default:\r\n                    input.skip(ftype)\r\n            }')
        } else {
        ...
        if (func['type'] !== 'void') {
            codes.push(`\r\n        if (this.success !== null && this.success !== undefined) {\r\n            output.writeFieldBegin('success', ${getThriftTypeStr(func['type'])}, 0)\r\n            output.${getWriterStr(func['type'])}(this.success)\r\n            output.writeFieldEnd()\r\n        }`)
        }
        if (throws && throws.length) {
            throws.forEach((item, index) => {
                codes.push(`\r\n        if (this.${item['name']} !== null && this.${item['name']} !== undefined) {\r\n            output.writeFieldBegin('${item['name']}', ${getThriftTypeStr(item['type'])}, ${index + 1})\r\n            this.${item['name']}.write(output)\r\n            output.writeFieldEnd()\r\n        }`)
            })
        }
